...
public class OdontologoJpaController implements Serializable {
    ...
    public OdontologoJpaController() {
        emf = Persistence.createEntityManagerFactory("Clinica_odontologica_PU");
    }
    ...
    public Odontologo obtenerOdontólogoPorUsuario(String nombreUsuario) {
        
        EntityManager em = getEntityManager();

        try {
            //Crear y ejecutar una consulta para obtener el usuario por nombre de usuario
            TypedQuery<Odontologo> query = em.createQuery(
                    
      "SELECT od FROM Odontologo od WHERE od.unUsuario.nombre_usuario = :nombreUsuario", Odontologo.class);
